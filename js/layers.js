...
const UNI_PHOTONS_ORDER = ["无线电波","微波","红外线","橙光","黄光","X射线","伽马射线"]
const UNI_PHOTONS_COLOR = ["#444444","brown","red",'orange','yellow']
...
addLayer("Uni", {
    ...
    buyables: {
        ...
        'ph6': {
            ...
            display() {if(n(player.Uni.photonsP).gte(28))return '进一步提升夸克与光子获取乘数。<br><br>当前等级：'+getBuyableAmount(this.layer,this.id)+'<br>'+'夸克 ×'+format(this.effect())+quickColor("(基数：×"+format(this.base())+")","red")+"<br>费用："+formatWhole(this.cost())+" 光子精华"
        else return '红外线2层解锁'},
        ...
        'qkc2': {
            ...
            display() {return '夸克降低至原来的三分之一，但持有的宇宙精华立即翻倍。<br><br>点击以激活该过滤器！'},
            canAfford() {return player.Uni.quarks.gte(1)},
            base(){
                let base = n(10)
                return base
            },
    ...
    getQuarkGain(){
        ...
        if(player.Uni.feature >= 3) gain = gain.mul(layers.Uni.getElecGain())
        return gain
    },
    getColoredQuarkGain(){
        if(!layers.Uni.clickables['qk1'].canClick()) return n(0)
    ...
    challenges:{
        ...
        'qk3':{
            ...
            color: '#2255FF',
            challengeDescription() {
                let desc = "↑↑点击夸克图标以开始夸克共振！<br>———————————————————————<br>共振效果：1.共振阶层不再提升光子获取，而是超级加倍降低光子获取。<br> 2.光子溢出指数固定为x<sup>0.191981</sup>。<div style='background-color:#2255FF'>夸克共振需求：夸克≥10<sup>6</sup></div>"
        ...
        'qk4':{
            ...
            color: '#55FFFF',
            challengeDescription() {
                let desc = "↑↑点击夸克图标以开始夸克共振！<br>———————————————————————<br>共振效果：1.提升阶层所需光子扩张10000倍。<br> 2.光子的一重溢出重新生效且效应变得超强。<br>3.注：该种夸克无论如何最多只能获得一个。<div style='background-color:#55FFFF'>夸克共振需求：夸克≥5×10<sup>8</sup></div>"
        ...
        'qk5':{
            ...
            color: '#FF55FF',
            challengeDescription() {
                let desc = "↑↑点击夸克图标以开始夸克共振！<br>———————————————————————<br>共振效果：1.所有光子理论效果降低到^0.25。<br> 2.共振阶层不再提升光子获取，而是究级加倍降低光子获取。<div style='background-color:#FF55FF'>夸克共振需求：夸克≥10<sup>12</sup></div>"
        ...
        'qk6':{
            ...
            color: '#FFFF55',
            challengeDescription() {
                let desc = "↑↑点击夸克图标以开始夸克共振！<br>———————————————————————<br>共振效果：1.所有光子理论效果降低到^0.1。<br> 2.共振阶层不再提升光子获取，而是超究加倍降低光子获取。<div style='background-color:#FFFF55'>夸克共振需求：夸克≥10<sup>20</sup></div>"
    ...
    tabFormat:{
        "Pre-Hydrogen":{
                content:[
                    ...
                    ["column", [["raw-html", function() {}],
                     "blank",['display-text',function(){return '<h3>[阶段0-4]-电子<br>此阶段的每个成就暂时没有额外效果！ '}],
